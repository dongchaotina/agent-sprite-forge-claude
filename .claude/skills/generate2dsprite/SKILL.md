...
description: "Generate and postprocess general 2D game assets and animation sheets: pixel-art sprites, clean HD map props, creatures, characters, NPCs, spells, projectiles, impacts, props, summons, and transparent GIF exports. Use when Claude should infer the asset plan from a natural-language request, match a reference or map art style, call an image generation tool for solid-magenta raw sheets, and use the local processor only for chroma-key cleanup, frame extraction, alignment, QC, and transparent exports."
---
...
> Image generation: Claude Code has no built-in raster image generator, so this skill ships its own. `scripts/gen_image.py` (OpenAI `gpt-image-1`) is the image generation tool — it produces the raw magenta-background sheet that `scripts/generate2dsprite.py` then postprocesses. It needs the `openai` package and an `OPENAI_API_KEY` environment variable. The deterministic processor (`generate2dsprite.py`) only needs Python plus Pillow/NumPy.
...
- Use `scripts/gen_image.py` for every raw image.
- Do not create raw sprite art with Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots. Runtime code may display finished generated assets, and scripts may make layout guides or postprocess generated images, but requested sprite art must originate from `scripts/gen_image.py`.
- When the user provides or implies a visual reference, pass it to `scripts/gen_image.py --reference <path>`. View the reference with the Read tool first so you can describe its invariants in the prompt; do not rely on a filesystem path in the prompt text as the visual reference.
...
- Make the reference visible first. For local paths, view the image with the Read tool; for freshly generated references, rely on the image already shown in context.
...
- Do not ask `scripts/gen_image.py` to generate unrelated action rows in one raw sheet, such as `row 1 idle, row 2 run, row 3 shoot, row 4 jump`, for a controllable hero or main character.
- Do not ask `scripts/gen_image.py` to generate raw single-row action strips such as `1x4 idle`, `1x4 run`, `1x4 shoot`, or `1x4 jump` for a controllable hero, character, creature, NPC, enemy, summon, or animated prop.
...
If a layout guide is useful, generate one before calling `scripts/gen_image.py`:
...
Then pass the guide to `scripts/gen_image.py --reference <guide.png>` and state in the prompt that the guide is layout-only: use it for invisible slot count, spacing, centering, and safe padding. The output must not reproduce guide boxes, safe-area rectangles, center marks, labels, borders, or guide background.
...
Run `scripts/gen_image.py` with the prompt you wrote:

```bash
python scripts/gen_image.py \
  --prompt "<your hand-written prompt>" \
  --size 1024x1024 \
  --output <run-dir>/raw-sheet.png
```

Pick `--size` from the sheet aspect: square grids -> `1024x1024`, wide strips -> `1536x1024`, tall sheets -> `1024x1536`.

When a reference image or layout guide is involved, pass it with `--reference` (repeatable); the script then uses the image-edit endpoint:

```bash
python scripts/gen_image.py \
  --prompt "<your hand-written prompt>" \
  --reference <reference-or-guide.png> \
  --output <run-dir>/raw-sheet.png
```
...
The script saves the PNG at `--output`, writes a `<output>.prompt.txt` sidecar, and prints the saved path. Keep the original generated image in place and run the processor on a copy or by path.
...
- `scripts/gen_image.py`: image generation tool (OpenAI `gpt-image-1`) for raw magenta-background sheets; supports `--reference` for reference-guided edits and layout guides
- `scripts/generate2dsprite.py`: postprocess primitive for cleanup, extraction, alignment, QC, and GIF export
- `scripts/make_layout_guide.py`: deterministic layout-guide image generator for geometry references
