...
- raw sprite art must come from `scripts/gen_image.py`, not Three.js, Canvas, SVG, HTML/CSS drawing, PIL shape drawing, procedural geometry, placeholder primitives, or code-rendered screenshots
...
- Pass the reference image to `scripts/gen_image.py --reference <path>`. View it with the Read tool first so you can describe its invariants in the prompt; do not assume a bare path string in the prompt text is a visual input.
